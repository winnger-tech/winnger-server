  ...
  async login(req, res) {
    try {
      ...
      if (driver.isRegistrationComplete) {
        ...
        const stageInfo = this.getNextStageInfo(driver.registrationStage, driver);
        if (stageInfo && stageInfo.title) {
          stageMessage = `You are currently on Stage ${driver.registrationStage}: ${stageInfo.title}. ${stageInfo.description}`;
        } else {
