...
describe('Driver Registration API', () => {
  ...
  describe('POST /api/drivers', () => {
    it('should register a new driver with valid data', async () => {
      ...
      const res = await request(app)
        .post('/api/drivers')
        .field('fullName', validDriver.fullName)
        .field('email', validDriver.email)
        .field('phone', validDriver.phone)
        .field('address', JSON.stringify(validDriver.address))
        .attach('aadharCard', Buffer.from('test'), { filename: 'aadhar.pdf', contentType: 'application/pdf' })
        .attach('drivingLicense', Buffer.from('test'), { filename: 'license.pdf', contentType: 'application/pdf' });
    ...
    it('should not register a driver with existing email', async () => {
      ...
      Driver.findOne.mockResolvedValue({ email: validDriver.email });
  ...
  describe('PUT /api/drivers/:id/payment', () => {
    ...
    it('should update payment status successfully', async () => {
      ...
      const res = await request(app)
        .put(`/api/drivers/${mockDriver._id}/payment`)
        .send(paymentData);
    ...
    it('should not update payment for non-existent driver', async () => {
      ...
      const res = await request(app)
        .put('/api/drivers/nonexistent-id/payment')
        .send({
          transactionId: 'test_tx_123',
          amount: 50
        });
  ...
  describe('PUT /api/drivers/:id/status', () => {
    ...
    it('should update driver status successfully', async () => {
      ...
      const res = await request(app)
        .put(`/api/drivers/${mockDriver._id}/status`)
