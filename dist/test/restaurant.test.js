...
describe('Restaurant Registration API', () => {
  ...
  describe('POST /api/restaurants', () => {
    ...
    it('should register a new restaurant with valid data', async () => {
      ...
      const res = await request(app).post('/api/restaurants').field('ownerName', validRestaurant.ownerName).field('email', validRestaurant.email).field('phone', validRestaurant.phone).field('restaurantName', validRestaurant.restaurantName).field('address', JSON.stringify(validRestaurant.address)).attach('fssai', Buffer.from('test'), {
        filename: 'fssai.pdf',
        contentType: 'application/pdf'
      }).attach('gst', Buffer.from('test'), {
        filename: 'gst.pdf',
        contentType: 'application/pdf'
      }).attach('pan', Buffer.from('test'), {
        filename: 'pan.pdf',
        contentType: 'application/pdf'
      }).attach('businessLicense', Buffer.from('test'), {
    ...
    it('should require all mandatory fields', async () => {
      const res = await request(app).post('/api/restaurants').field('email', validRestaurant.email).attach('fssai', Buffer.from('test'), {
        filename: 'fssai.pdf',
        contentType: 'application/pdf'
      }).attach('gst', Buffer.from('test'), {
        filename: 'gst.pdf',
  ...
  describe('PUT /api/restaurants/:id/payment', () => {
    ...
    it('should update payment status successfully', async () => {
      ...
      const res = await request(app).put(`/api/restaurants/${mockRestaurant._id}/payment`).send(paymentData);
    ...
    it('should not update payment for non-existent restaurant', async () => {
      ...
      const res = await request(app).put('/api/restaurants/nonexistent-id/payment').send({
        transactionId: 'test_tx_123',
        amount: 100
      });
  ...
  describe('PUT /api/restaurants/:id/status', () => {
    ...
    it('should update restaurant status successfully', async () => {
      Restaurant.findById.mockResolvedValue(mockRestaurant);
      const res = await request(app).put(`/api/restaurants/${mockRestaurant._id}/status`).send({
        status: 'approved'
      });
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
