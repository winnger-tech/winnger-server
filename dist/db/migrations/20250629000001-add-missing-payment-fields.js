...
module.exports = {
  ...
  down: async (queryInterface, Sequelize) => {
    await queryInterface.sequelize.transaction(async transaction => {
      ...
      await queryInterface.removeColumn('restaurants', 'stripePaymentMethodId', {
        transaction
      });
      await queryInterface.removeColumn('restaurants', 'pendingPaymentIntentId', {
        transaction
      });
      await queryInterface.removeColumn('restaurants', 'paymentCompletedAt', {
        transaction
      });
      await queryInterface.removeColumn('restaurants', 'registrationCompletedAt', {
        transaction
      });

      // Remove review & confirmation fields
      await queryInterface.removeColumn('restaurants', 'agreedToTerms', {
        transaction
      });
      await queryInterface.removeColumn('restaurants', 'confirmationChecked', {
        transaction
      });
      await queryInterface.removeColumn('restaurants', 'additionalNotes', {
        transaction
      });
      await queryInterface.removeColumn('restaurants', 'reviewCompletedAt', {
        transaction
      });
